...
*/
	switch ( header->msgId )
	{
	case MsgId::createData://MSG_HEADER+数据pData(CREATE_DATA+path)
...
void NoDB::ReadDataFromSSD( exist::VALUE *pValue, unsigned char *path, int size )
{
	...
	{//创建数据:MSG_HEADER+CREATE_DATA+path
		...
		{
			pSSD->Close();
			m_log.Info( "Error", "固态硬盘接触不良，无法创建持久化数据" );
			return;
		}
	}
	if ( DataType::IsContainer(pValue->key.type) ) return;//容器类数据存储，请期待

	{//查询数据:MSG_HEADER+DATA_KEY+path
